...
class iGrow_Macon_Utilities {
	...
	public function add_mime_types( $post_mime_types ) {
		...
		$post_mime_types['application/pdf'] = array( esc_html__( 'PDFs', 'igrow-macon' ), esc_html__( 'Manage PDFs', 'igrow-macon' ), _n_noop( 'PDF <span class="count">(%s)</span>', 'PDFs <span class="count">(%s)</span>' ) );
		$post_mime_types['text/x-vcard'] 	= array( esc_html__( 'vCards', 'igrow-macon' ), esc_html__( 'Manage vCards', 'igrow-macon' ), _n_noop( 'vCard <span class="count">(%s)</span>', 'vCards <span class="count">(%s)</span>' ) );
	...
	public function async_scripts( $tag, $handle ) {
		...
		$check = strpos( $handle, 'igrowmacon-' );
	...
	public function background_images() {
		...
		$output = '';
		$image 	= igrowmacon_get_thumbnail_url( get_the_ID(), 'full' );
	...
	public function category_transient_flusher() {
		...
		// Like, beat it. Dig?
		delete_transient( 'igrowmacon_categories' );
	...
	public function excerpt_read_more( $more ) {
		...
		$return .= esc_html__( 'Read more', 'igrow-macon' );
		$return .= '<span class="screen-reader-text">';
		$return .= sprintf( esc_html__( ' about %s', 'igrow-macon' ), $post->post_title );
		$return .= '</span></a>';
	...
	public function page_template_column_content( $column_name, $post_ID ) {
		...
		if ( ! empty( $name ) ) {
			...
			echo '<span class="name-template">' . esc_html( 'Default', 'igrow-macon' ) . '</span>';
	...
	public function page_template_column_head( $defaults ) {
		...
		$defaults['page_template'] = esc_html( 'Page Template', 'igrow-macon' );
