...
class iGrow_Macon_Authorbox {
	...
	public function hooks() {
		...
		add_action( 'igrowmacon_authorbox', array( $this, 'wrap_begin' ), 5, 1 );
		add_action( 'igrowmacon_authorbox', array( $this, 'avatar' ), 15, 1 );
		add_action( 'igrowmacon_authorbox', array( $this, 'name' ), 25, 1 );
		add_action( 'igrowmacon_authorbox', array( $this, 'bio' ), 35, 1 );
		add_action( 'igrowmacon_authorbox', array( $this, 'posts_link' ), 45, 1 );
		add_action( 'igrowmacon_authorbox', array( $this, 'social_links' ), 55, 1 );
		add_action( 'igrowmacon_authorbox', array( $this, 'wrap_end' ), 95, 1 );
	...
	public function avatar( $authordata ) {
		...
		?><div class="authorbox-avatar"><?php
			...
			/**
			 * The igrowmacon_authorbox_avatar_size filter.
			 * @var 	int 		The avatar size.
			 */
			$size = apply_filters( 'igrowmacon_authorbox_avatar_size', 75 );

			/**
			 * The igrowmacon_authorbox_default_avatar filter.
			 * @var 	string 		The default avatar image.
			 */
			$img = apply_filters( 'igrowmacon_authorbox_default_avatar', '' );

			/**
			 * The igrowmacon_authorbox_avatar_alttext filter.
			 * @var 	string 		The alt text for the avatar.
			 */
			$alt = apply_filters( 'igrowmacon_authorbox_avatar_alttext', $authordata->nickname );

			/**
			 * The igrowmacon_authorbox_avatar_args filter.
			 * @var [type]
			 */
			$avatar_args['class'] 			= 'authorbox-avatar-img';
			 $avatar_args['extra_attr'] 	= 'itemprop="image"';
			$args 							= apply_filters( 'igrowmacon_authorbox_avatar_args', $avatar_args );
	...
	protected function get_socials_list() {
		...
		$socials['facebook'] 	= __( 'Facebook', 'igrow-macon' );
		$socials['twitter'] 	= __( 'Twitter', 'igrow-macon' );
		$socials['googleplus'] 	= __( 'Google Plus', 'igrow-macon' );
		$socials['linkedin'] 	= __( 'LinkedIn', 'igrow-macon' );
		$socials['youtube'] 	= __( 'YouTube', 'igrow-macon' );
		$socials['videmo'] 		= __( 'Vimeo', 'igrow-macon' );
		$socials['wordpress'] 	= __( 'WordPress', 'igrow-macon' );
		$socials['pinterest'] 	= __( 'Pinterest', 'igrow-macon' );
		$socials['instagram'] 	= __( 'Instagram', 'igrow-macon' );

		/**
		 * The igrowmacon_authorbox_socials filter.
		 * @var 		array 		$socials
		 */
		$socials = apply_filters( 'igrowmacon_authorbox_socials', $socials );
	...
	public function posts_link( $authordata ) {
		...
		?><a class="authorbox-postslink" href="<?php echo esc_url( get_author_posts_url( $authordata->ID ) ); ?>" rel="author"><?php
			...
			printf(
				...
				/**
				 * The igrowmacon_authorbox_moreposts_text filter.
				 * @var 		string 			The "more posts by" text for the authorbox.
				 */
				apply_filters( 'igrowmacon_authorbox_moreposts_text', __( 'More posts by %s', 'igrow-macon' ) ),
	...
	public function social_link( $authordata, $key, $social ) {
		...
		?><li class="authorbox-social">
			<a class="authorbox-social-link" href="<?php echo esc_url( $authordata->$key ); ?>" itemprop="url"><?php
				...
				igrowmacon_the_svg( $key );

				echo esc_html( $social, 'igrow-macon' );
